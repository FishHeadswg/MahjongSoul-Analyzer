...
(function () {
    ...
    class Analyzer {
        ...
        reinit() {
            ...
            this.thirtO = 0; // closed
            this.dragons = 0;
            this.simples = 0;
            this.seatWind = 0;
            this.prevWind = 0;
            this.pinfu = 0; // closed
            this.pureDbl = 0; // closed
            this.tripTrips = 0;
            this.tripQuads = 0;
            this.allTrips = 0;
            this.l3drags = 0;
            ...
            this.fullFlush = 0;
            this.bThreeDrgs = 0;
            this.fourConcealedTrips = 0;
            this.allHonors = 0;
            this.allGreen = 0;
            this.allTerminals = 0;
            this.fourLWinds = 0;
            this.fourQuads = 0;
            this.nineGates = 0; //closed
            this.FourCTrips = 0; //closed
            this.fourBWinds = 0;
            this.auto = false;
            this._handAnalyzer = +localStorage.handAnalyzer || 0;
        ...
        set handAnalyzer(i) {
            this._handAnalyzer = i;
            localStorage.handAnalyzer = i;
        ...
        // Apply analysis after each action type
        capture() {
            if (typeof uiscript === "undefined" || !uiscript.UI_DesktopInfo || typeof Laya.View.uiMap === "undefined" || !Laya.View.uiMap["mj/desktopInfo"]) return setTimeout(() => this.capture(), 1000);
            if (typeof view === "undefined" || !view.DesktopMgr || !view.DesktopMgr.prototype) return setTimeout(() => this.capture(), 1000);
            ...
            for (const key in actionsToInject) {
                ...
                mToInject.forEach(mType => {
                    ...
                    action[mType] = action => {
                        const r = m(action);
                        setTimeout(() => this.analyze(key, action, mType), delay + (key === "ActionNewRound" && action.al ? 1300 : 0));
                        return r;
                    }
                })
            }
            ...
            view.DesktopMgr.prototype.setChoosedPai = e => {
                ...
                if (e !== null) this.colorPool(e); // override rendering
                return r;
            }
            console.log("MahjongSoul Analyzer initialized.");
        }
        // Open analyzer from button
        appendEx() {
            const b = document.createElement("button");
            b.style.border = "none";
            b.style.background = "transparent";
            b.style.color = "white";
            b.innerText = "OPEN ANALYZER";
            b.style.position = "absolute";
            b.style.top = "0px";
            b.style.right = "0px";
            b.style.zIndex = 1000;
            b.addEventListener("click", () => {
                this.window = window.open("", "ANALYZER", "directories=no,titlebar=no,toolbar=no,location=no,status=no,menubar=no,scrollbars=no,resizable=yes,width=380,height=800");
                const doc = this.window.document;
                doc.body.innerHTML = "";
                doc.head.innerHTML = "";
                doc.write(`<html>
                        <head>
                            <title>MahjongSoul Analyzer</title>
                            <style>
                                .mask {
                                    position: relative;
                        ...
                        <body style="margin: 0px; background-color: black; color: #ccc">
                            ...
                            <p style="margin: 20px 0 0 12px; font-weight:bold; font-size:16px;" >Wait Tile Rates:</p>
                            <div id="waits" style="margin: 24px;">
                            </div>
                            <div id ="syaCnt" style="margin: 12px; font-weight:bold; font-size:16px;">Shanten Counts (Current = N/A): </div>
                            <table id="Syanten" style="margin: 24px;">
                                <tr><th>Yaku</th><th>Tiles Left (Until Tenpai)</th></tr>
                                <tr><td>Thirteen Orphans: </td><td id="13O">N/A</td></tr>
        ...
        sortTable() {
            ...
            table = this.window.document.getElementById("Syanten");
            switching = true;
            while (switching) {
                switching = false;
                rows = table.rows;
        ...
        analyze(key, action, mType) {
            ...
            if (key == "ActionDiscardTile") {
                ...
                this.defenseInfo.river[action.seat].push({ tileIndex: Analyzer.getTileIndex(tile.val.toString()), afterRiichi: this.defenseInfo.riichiPlayers.slice() });
            ...
            if (action.hasOwnProperty("operation")) {
                ...
                for (const operation of operations.operation_list) {
                    if (operation.type == 1) {
                        ...
                        const option = this.analyzeHand();
                        if (this.auto && typeof option === "number") setTimeout(() => this.discard(Analyzer.indexToString(option)), Math.random() * 2000 + 1000);
                    }
                }
            }
        }
        ...
        calcMountain() {
            ...
            const visibleTiles = [];
            view.DesktopMgr.Inst.players.forEach((player, i) => {
                ...
                for (const tile of player.container_qipai.pais) {
                    ...
                    if (reinitRiver) this.defenseInfo.river[seat].push({ tileIndex: Analyzer.getTileIndex(tile.val.toString()), afterRiichi: [] });
                }
                // Get babeis
                for (const tile of player.container_babei.pais) {
                    visibleTiles.push(tile.val.toString());
                }
                // Add last discarded tiles
                ...
                for (const tile of player.container_ming.pais) {
                    visibleTiles.push(tile.val.toString());
                    this.defenseInfo.fuuro[seat].push(Analyzer.getTileIndex(tile.val.toString()));
                }
            })
            // Get hand
            ...
            // Get dora tiles
            view.DesktopMgr.Inst.dora.forEach(tile => {
                visibleTiles.push(tile.toString());
            });
            // Remove visiable tiles form haiyama
            visibleTiles.forEach(strTile => this.mountain[Analyzer.getTileIndex(strTile)]--);
        ...
        displayMountain() {
            ...
            this.mountain.forEach((cnt, tile) => {
                d.getElementById(Analyzer.indexToString(tile)).style.backgroundColor = "rgba(0,0,0," + (4 - cnt) / 5 + ")";
                d.getElementById(Analyzer.indexToString(tile)).innerText = cnt.toString();
            })
        }
        displaySyanten() {
            if (!this.window) return;
            const d = this.window.document;
            d.getElementById("syaCnt").innerText = "Syanten Counts (Current = " + this.syaCount + "):";
            d.getElementById("7Pair").innerText = this.sevPair;
            d.getElementById("13O").innerText = this.thirtO;
            d.getElementById("dragons").innerText = this.dragons;
            this.sortTable();
        }
        displayRates(options) {
            if (!this.window) return;
            const doc = this.window.document;
            doc.getElementById("waits").innerHTML = "";
            const tb = doc.createElement("table");
            options.forEach((waitTile) => {
                let row = tb.insertRow(-1);
                let c0 = row.insertCell(0);
                let c1 = row.insertCell(1);
                c0.innerText = Analyzer.indexToReadableString(waitTile.tileIndex) + ": ";
                c1.innerText = parseFloat(waitTile.rate).toFixed(2);
                if (waitTile.rate >= .9) row.style.color = "green";
                else if (waitTile.rate <= .6) row.style.color = "red";
            })
            doc.getElementById("waits").appendChild(tb);
        }
        analyzeHand() {
            let option;
            if (!view.DesktopMgr.Inst || !view.DesktopMgr.Inst.mainrole.hand.length) return option;
            const hand = tenhou.MPSZ.exextract34(tenhou.MPSZ.expand(this.handToString()));
            const atkOptions = Analyzer.evalAtk(hand, this.mountain);
            this.displaySyanten();
            this.displayRates(atkOptions);
            const defOptions = Analyzer.evaluateDefense(hand, this.mountain, this.defenseInfo, view.DesktopMgr.Inst.player_datas.length);
            const atk = this._handAnalyzer < 2 ? 1 : 0;
            let maxRate = -1;
            view.DesktopMgr.Inst.mainrole.hand.forEach(tile => {
                const tileIndex = Analyzer.getTileIndex(tile.val.toString());
                let atkRate = 0;
                let defRate = 0;
        ...
        static evalAtk(hand, mountain) {
            // Check waits against mountain
            const restc = waitings => {
                let rest = 0;
                waitings.forEach(tileIndex => rest += mountain[tileIndex]); // make sure wait tile exists in mountain
                return rest;
            }
            const options = [];
            const syanten_org = tenhou.SYANTEN.calcSyanten2(hand, 34)[0]; // Get syanten #
            analyzer.syaCount = syanten_org;
            if (syanten_org == -1) return options; // Get out fgt
            else if (syanten_org == 0) { // Tenpai
                for (let i = 0; i < 34; i++) { // Transverse / Search
                    if (!hand[i]) continue;
                    hand[i]--; // Hit
                    const waitings = [];
                    for (let j = 0; j < 34; j++) {
                        if (i == j || hand[j] >= 4) continue;
                        hand[j]++; // Add to hand, check if it's a winning tile
                        if (tenhou.AGARI.isAgari(hand)) waitings.push(j);
                        hand[j]--;
                    }
                    hand[i]++;
                    if (waitings.length) options.push({ tileIndex: i, n: restc(waitings) }); // add to tile options if valid
                }
            }
            else { // The road to tenpai
                for (let i = 0; i < 34; i++) {
                    if (!hand[i]) continue;
                    hand[i]--; // Hit
                    const waitings = [];
                    for (let j = 0; j < 34; ++j) {
                        if (i == j || hand[j] >= 4) continue;
                        hand[j]++; //  Add to hand, check if it reduces syanten (syanten_org - 1)
            ...
            if (!options.length) return options; // OOF
            options.sort((a, b) => b.n - a.n); // sort by available waits
            let maxn = options[0].n; // best option
            options.forEach(option => option.rate = option.n / maxn); // rank relative to max
            this.findYaku(hand, mountain);
            return options;
        }
        static findYaku(hand, mountain) {
            let c = hand;
            let dragons = 99;
            for (let i = 31; i < 34; ++i) {
                switch (c[i]) {
                    ...
                    case 3:
                        ...
                        break;
                    case 2:
                        if (mountain[i] >= 1) dragons = 1;
                        else dragons = 99;
                        break;
                    case 1:
                        if (dragons === 1) break;
                        if (mountain[i] >= 2) dragons = 2;
                        else dragons = 99;
                        break;
                    default:
                        if (dragons >= 2 && dragons < 4) break;
                        if (mountain[i] > 2) dragons = 3;
                        break;
                }
                if (dragons === 0) break; // dragon yaku already found
            }
            analyzer.dragons = dragons;
        }
        initDefI() {
        ...
        colorPool(tileIn) {
            ...
            const warningColor = (tileSelected, tileRiverModel, isSelf) => {
                ...
                if (delta < 2) { // 壁
                    const tilesInMountain = this.mountain[Analyzer.getTileIndex(tileRiver.toString())];
        ...
        // Convert tile code into mountain index (1m --> 0)
        static getTileIndex(str) {
        ...
        static indexToReadableString(i) {
            let j = (i % 9 + 1) + "mpsz"[parseInt(i / 9)];
            if (j.substring(1, 2) === "z") {
                switch (j.substring(0, 1)) {
                    case "1":
                        j = "EAST";
                        break;
                    case "2":
            ...
            else if (j.substring(1, 2) === "m") j = j.substring(0, 1) + " MAN";
            else if (j.substring(1, 2) === "p") j = j.substring(0, 1) + " PIN";
            else j = j.substring(0, 1) + " SOU";
            return j;
        }
    }
    Analyzer.TILE_SAFETY_RATE = {
        Genbutsu: [1, 0], // 现物
        TankiZ: [0.9, 0], // 单骑字牌
    ...
    var MPSZ = {
        ...
        // Convert hand tiles to mountain type
        exextract34: function (t) {
            ...
            for (var i = 0; i < s.length; i += 2) {
                var strTile = s.substr(i, 2),
                    index = -1;
                if (strTile % 10) {
                    index = 9 * Math.floor(strTile / 10) + ((strTile % 10) - 1); // -1 + (-1 * tileType)
                } else {
                    index = 9 * strTile / 10 + 4; // red 5
                }
    ...
    var AGARI = { // Ron checking only
        ...
        isAgari: function (c) {
            var j = (1 << c[27]) | (1 << c[28]) | (1 << c[29]) | (1 << c[30]) | (1 << c[31]) | (1 << c[32]) | (1 << c[33]);
            if (j >= 0x10) return false; // 4 honor tiles
            // 13 Orphans
            ...
            // General hands
            if (j & 2) return false; // 1 honor tile
            var n00 = c[0] + c[3] + c[6],
                n01 = c[1] + c[4] + c[7],
                n02 = c[2] + c[5] + c[8];
            var n10 = c[9] + c[12] + c[15],
            ...
            if (n0 == 1) return false; // Extra man
            var n1 = (n10 + n11 + n12) % 3;
            if (n1 == 1) return false; // Extra pin
            var n2 = (n20 + n21 + n22) % 3;
            if (n2 == 1) return false; // Extra sou
            if ((n0 == 2) + (n1 == 2) + (n2 == 2) +
                (c[27] == 2) + (c[28] == 2) + (c[29] == 2) + (c[30] == 2) +
                (c[31] == 2) + (c[32] == 2) + (c[33] == 2) != 1) return false; // No valid pair / 2+ pairs
            ...
            var nn2 = (n20 * 1 + n21 * 2) % 3,
                m2 = this.cc2m(c, 18);
            if (j & 4) return !(n0 | nn0 | n1 | nn1 | n2 | nn2) && this.isMentsu(m0) && this.isMentsu(m1) && this.isMentsu(m2); // Honor pair
            if (n0 == 2) return !(n1 | nn1 | n2 | nn2) && this.isMentsu(m1) && this.isMentsu(m2) && this.isAtamaMentsu(nn0, m0); // Man pair
            if (n1 == 2) return !(n2 | nn2 | n0 | nn0) && this.isMentsu(m2) && this.isMentsu(m0) && this.isAtamaMentsu(nn1, m1); // Pin pair
            if (n2 == 2) return !(n0 | nn0 | n1 | nn1) && this.isMentsu(m0) && this.isMentsu(m1) && this.isAtamaMentsu(nn2, m2); // Sou pair
            return false;
        }
    }
    ...
    var SYANTEN = { // singleton
        ...
        scanChiitoiKokushi: function () {
            ...
            var c = e.c;
            var n13 = // Terminal pair+ count
                (c[0] >= 2) + (c[8] >= 2) +
                (c[9] >= 2) + (c[17] >= 2) +
                (c[18] >= 2) + (c[26] >= 2) +
                (c[27] >= 2) + (c[28] >= 2) + (c[29] >= 2) + (c[30] >= 2) + (c[31] >= 2) + (c[32] >= 2) + (c[33] >= 2);
            var m13 = // Check all terminals for 13 orphans
                (c[0] != 0) + (c[8] != 0) +
                (c[9] != 0) + (c[17] != 0) +
                (c[18] != 0) + (c[26] != 0) +
                (c[27] != 0) + (c[28] != 0) + (c[29] != 0) + (c[30] != 0) + (c[31] != 0) + (c[32] != 0) + (c[33] != 0);
            var n7 = n13 + // Check non-terminals for 7 pairs
            ...
            var m7 = m13 + // Check non-terminals for 13o
            ...
            { // 7pair
                ...
                analyzer.sevPair = ret_syanten;
                if (ret_syanten < syanten) syanten = ret_syanten;
            }
            { // 13o
                var ret_syanten = 13 - m13 - (n13 ? 1 : 0);
                analyzer.thirtO = ret_syanten;
                if (ret_syanten < syanten) syanten = ret_syanten;
            }
            return syanten;
        },
        removeJihai: function (nc) { // honor tiles
            var e = this;
            var c = e.c;
            var j_n4 = 0; // 7-bit honor tiles
            var j_koritsu = 0; // isolated honor tiles
            var i;
            ...
            if (j_koritsu) { // if isolated tiles exist
                e.f_koritsu |= (1 << 27); // 1000 0000 0000 0000 0000 0000 0101
                if ((j_n4 | j_koritsu) == j_n4) e.f_n4 |= (1 << 27); // isolated tile available to form a pair doesn't exist (being used for kan)
            }
        },
        ...
        scanNormal: function (init_mentsu) {
            var e = this;
            var c = e.c;
            e.f_n4 |= // Isolated number tile that can not be a pair to complete the hand (kan)
            ...
            this.n_mentsu += init_mentsu; // # mentsu already completed
            e.Run(0);
        },

        Run: function (depth) { // Nesting is at most 14 deep (works by removing tiles for each found meld and then running against remaining tiles)
            var e = this;
            ++e.n_eval;
            if (e.min_syanten == -1) return; // Already in tenpai
            var c = e.c;
            for (; depth < 27 && !c[depth]; ++depth); // skip non-existant tiles
            if (depth == 27) return e.updateResult(); // skip honors
            var i = depth;// i = tile number
            if (i > 8) i -= 9;
            if (i > 8) i -= 9; // mod_9_in_27
            switch (c[depth]) { // c[0] = 1m
                case 4:
                    // Pon + Chii | Tatsu | Isolated
                    e.i_anko(depth); // Pon
                    if (i < 7 && c[depth + 2]) { // e.g. 2m4m, ignore 8/9
                        if (c[depth + 1]) e.i_syuntsu(depth), e.Run(depth + 1), e.d_syuntsu(depth); // Chii + check chii for each tile in meld
                        e.i_tatsu_k(depth), e.Run(depth + 1), e.d_tatsu_k(depth); // Middle wait
                    }
                    if (i < 8 && c[depth + 1]) e.i_tatsu_r(depth), e.Run(depth + 1), e.d_tatsu_r(depth); // Double-sided wait (2m3m) or side wait (8m9m)
                    e.i_koritsu(depth), e.Run(depth + 1), e.d_koritsu(depth); // Orphan tile
                    e.d_anko(depth); // Add pon back
                    // Pair + Chii // Orphan remaining? // Pair + Pair not allowed
                    e.i_toitsu(depth);
                    if (i < 7 && c[depth + 2]) {
                        if (c[depth + 1]) e.i_syuntsu(depth), e.Run(depth), e.d_syuntsu(depth); // Chii + other
                        e.i_tatsu_k(depth), e.Run(depth + 1), e.d_tatsu_k(depth); // No need for 2 pairs
                    }
                    if (i < 8 && c[depth + 1]) e.i_tatsu_r(depth), e.Run(depth + 1), e.d_tatsu_r(depth);
                    e.d_toitsu(depth);
                    break;
                case 3:
                    // Only concealed triplet
                    e.i_anko(depth), e.Run(depth + 1), e.d_anko(depth);
                    // Pair + Chii | Tatsu
                    e.i_toitsu(depth);
                    if (i < 7 && c[depth + 1] && c[depth + 2]) {
                        e.i_syuntsu(depth), e.Run(depth + 1), e.d_syuntsu(depth); // Chii
                    } else { // If chii removed, tatsu is left
                        if (i < 7 && c[depth + 2]) e.i_tatsu_k(depth), e.Run(depth + 1), e.d_tatsu_k(depth); // Double middle waits is unneeded -> same as 2 pairs
                        if (i < 8 && c[depth + 1]) e.i_tatsu_r(depth), e.Run(depth + 1), e.d_tatsu_r(depth); // Double-sided wait
                    }
                    e.d_toitsu(depth);
                    // Chii
                    if (i < 7 && c[depth + 2] >= 2 && c[depth + 1] >= 2) e.i_syuntsu(depth), e.i_syuntsu(depth), e.Run(depth), e.d_syuntsu(depth), e.d_syuntsu(depth); // Chii + other
                    break;
                case 2:
                    // Only pair
                    e.i_toitsu(depth), e.Run(depth + 1), e.d_toitsu(depth);
                    // Chii
                    if (i < 7 && c[depth + 2] && c[depth + 1]) e.i_syuntsu(depth), e.Run(depth), e.d_syuntsu(depth); // Chii + Other
                    break;
                case 1:
                    // It's not necessary to have 2+ orphans -> Pairs will make the number of turns decrease
                    if (i < 6 && c[depth + 1] == 1 && c[depth + 2] && c[depth + 3] != 4) { // Nobetan (pair either side of 4-tile chii)
                        e.i_syuntsu(depth), e.Run(depth + 2), e.d_syuntsu(depth); // Chii + 1
                    } else {
                        //				if (n_koritsu<8) e.i_koritsu(depth), e.Run(depth+1), e.d_koritsu(depth);
                        e.i_koritsu(depth), e.Run(depth + 1), e.d_koritsu(depth);
                        // Chii
                        if (i < 7 && c[depth + 2]) {
                            if (c[depth + 1]) e.i_syuntsu(depth), e.Run(depth + 1), e.d_syuntsu(depth); // Chii + Other
                            e.i_tatsu_k(depth), e.Run(depth + 1), e.d_tatsu_k(depth); // Double middle waits is unneeded -> same as 2 pairs
                        }
                        if (i < 8 && c[depth + 1]) e.i_tatsu_r(depth), e.Run(depth + 1), e.d_tatsu_r(depth); // Terminal chii
                    }
                    break;
            }
        },
        ...
        calcSyanten2(a, n) {
            ...
            e.init(a, n);
            var nc = e.count34();
            if (nc > 14) return undefined;
            var syanten = [e.min_syanten, e.min_syanten];
            if (nc >= 13) syanten[0] = e.scanChiitoiKokushi(nc);
            e.removeJihai(nc);
            var init_mentsu = Math.floor((14 - nc) / 3); // Account for called/open melds in hand
            e.scanNormal(init_mentsu); // Find lowest syanten
